...
### 金融单品追踪（场景五）
围绕某企业的单一产品做持续追踪，发现潜在交易机会：
- **实体结构**：话题 `meta.financial` 记录公司 / 股票代码 / 单品 / 业务线 / 价格带。
- **单品势能**：社媒声量·情绪·口碑·互动 → 单品健康度 + 热度分(0-100)。
- **热品榜**：同公司多单品聚合排名（矩阵热品）+ 跨市场热品榜。
- **交易机会**：单品信号 → 盈利映射 → 股价背离检测 → 催化剂日历 → 机会评分(0-100) / 方向 / 触发条件 / 风险。
- **金融数据**：股价/估值/交付量由采集阶段经 websearch 抓取东方财富/雪球/公告（或接入金融 MCP）写入 `meta.financial` 时点字段；缺失时自动降级为社媒信号并标注数据缺口。
- **看板**：网站独立「热品榜」页（`/hot-products`）展示矩阵热品、跨市场热品与交易机会卡片。
...
关注「莫说闲话」公众号，获取持续舆情追踪与深度报告：

![莫说闲话公众号](assets/qr_small.png)
